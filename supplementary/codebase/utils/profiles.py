...
class StepProfile:
    ...
    def generalize(self) -> Callable[[np.ndarray, float], np.ndarray]:
        ...
        def profile(x: np.ndarray, t: float) -> np.ndarray:
            return self.minimum + (self.maximum - self.minimum) * 0.5 * (
                1 + self.direction * np.tanh((x - self.offset) / self.epsilon)
            )
    ...
    def check_if_populated(self) -> None:
        if self.minimum is None or self.maximum is None or self.offset is None:
            raise ValueError(
                "Populate parameters using .populate_limits() or .populate_rho() before calling"
            )
...
class ExponentialProfile:
    ...
    def generalize(self) -> Callable[[np.ndarray, float], np.ndarray]:
        ...
        def profile(x: np.ndarray, t: float) -> np.ndarray:
            return (self.beta * np.exp(self.direction * self.beta * x)) / (
                self.direction * (np.exp(self.direction * self.beta) - 1)
            )
