...
class TemporalSolver:
    def __init__(
        ...
        self.domain = (
            mesh.create_interval(MPI.COMM_SELF, domain_resolution, [0.0, 1.0])
            if domain is None
            else domain
        )
        self.order = order
        self.functionspace = (
            fem.functionspace(self.domain, ("CG", self.order))
        ...
        self.update_delta = (
            (lambda x, t: np.ones_like(x)) if update_delta is None else update_delta
        )
        self.update_kappa = (
            (lambda x, t: np.ones_like(x)) if update_kappa is None else update_kappa
        )
        self.update_stomata = (
            (lambda x, t: np.ones_like(x)) if update_stomata is None else update_stomata
        )
        self.update_atmospheric = (
            (lambda x, t: np.ones_like(x))
            if update_atmospheric is None
            else update_atmospheric
        )
    ...
    def _setup_domain(self) -> tuple[ufl.Measure, ufl.Measure]:
        ...
        fdim = tdim - 1
        facets_left = mesh.locate_entities(
            self.domain, fdim, lambda x: np.isclose(x[0], 0.0)
        )
        facets_right = mesh.locate_entities(
            self.domain, fdim, lambda x: np.isclose(x[0], 1.0)
        )
        facet_indices = np.concatenate([facets_left, facets_right])
        facet_tags = np.concatenate(
            [np.full_like(facets_left, 1), np.full_like(facets_right, 2)]
        )
    ...
    def _setup_plotting(
        self, chi_n: fem.Function
    ) -> tuple[np.ndarray, pyvista.UnstructuredGrid, pyvista.Plotter]:
        ...
        grid.points = np.c_[
            xcoords, grid.point_data["chi"], np.zeros_like(xcoords)
        ]  # x, chi, 0
        ...
        homogeneous.points = np.c_[
            xcoords, self.initial_condition, np.zeros_like(xcoords)
        ]
        _ = plotter.add_mesh(
            homogeneous, color="black", show_edges=False, lighting=False, line_width=2
        )
    ...
    def solve(self) -> tuple[np.ndarray, np.ndarray]:
        ...
        chi_initial = np.interp(
            x,
            x,
            self.initial_condition,
            left=self.initial_condition[0],
            right=self.initial_condition[-1],
        )
        ...
        d, k, s, ca = (
            ...
            stomata_t,
            atmospheric_t,
        )  # aliases for readability in forms

        a = (
            chi * v * dx
            + dt * tau2 * k * chi * v * dx
            + dt * d * ufl.inner(ufl.grad(chi), ufl.grad(v)) * dx
            + dt * gamma * s * chi * v * ds(1)
        )  # left boundary at lambda = 0

        L = (
            chi_n * v * dx
            + dt * tau2 * k * chi_ * v * dx
            + dt * gamma * s * ca * v * ds(1)
        )  # left boundary at lambda = 0
        ...
        for n in range(num_steps):
            ...
            delta_t.x.array[:] = self.update_delta(x, t)
            delta_t.x.scatter_forward()
            kappa_t.x.array[:] = self.update_kappa(x, t)
            kappa_t.x.scatter_forward()
            stomata_t.x.array[:] = self.update_stomata(x, t)
            stomata_t.x.scatter_forward()
            atmospheric_t.x.array[:] = self.update_atmospheric(x, t)
            atmospheric_t.x.scatter_forward()
            ...
            if self.animate:
                ...
                grid.points[:, :] = np.c_[
                    xcoords,  # type: ignore[reportArgumentType]
                    chi_new.x.array,
