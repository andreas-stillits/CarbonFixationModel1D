...
Scan over (tau, gamma) space to reproduce figure 3C for nonlinear Rubisco

"""

from codebase.nonlinear.solver import NonlinearSolver
            ...
            for idx2 in range(c.n_rho):
                for idx3 in range(c.n_rho):
                    ...
                    solver = NonlinearSolver(
                        params, mu, delta=delta.steadify(), kappa=kappa.steadify()
                    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    if rank == 0:
        print(
            f"Running with {size} rank(s) and mu={args.mu} for nonlinear exploration of figure 3C",
            flush=True,
        )
