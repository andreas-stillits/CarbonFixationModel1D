...
class NonlinearSolver:
    def __init__(
        ...
        self.domain = (
            mesh.create_interval(MPI.COMM_SELF, self.domain_resolution, [0.0, 1.0])
            if domain is None
            else domain
        )
        self.order = order
    ...
    def solve(self) -> tuple[np.ndarray, np.ndarray]:
        ...
        chi = fem.Function(self.functionspace, name="chi")
        chi = cast(fem.Function, chi)
        ...
        # build forms
        F = (
            -delta * ufl.inner(ufl.grad(chi), ufl.grad(v)) * dx
