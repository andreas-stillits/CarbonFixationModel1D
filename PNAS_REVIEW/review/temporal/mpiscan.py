...
class System():
    def __init__(self, constants: TemporalExploration, case: str, quantity: str) -> None:
    ...
    def get_solvers(self, period: float, amplitude: float) -> tuple[TemporalSolver, TemporalSolver]:
        ...
        if self.quantity == "Ca":
            solver_hom = TemporalSolver(params, timing=timing, update_atmospheric=oscillator)
            solver_het = TemporalSolver(params, timing=timing, update_atmospheric=oscillator,
                                        update_delta=delta, update_kappa=kappa)
        elif self.quantity == "gs":
            solver_hom = TemporalSolver(params, timing=timing, update_stomata=oscillator)
            solver_het = TemporalSolver(params, timing=timing, update_stomata=oscillator,
                                        update_delta=delta, update_kappa=kappa)
            ...
            solver_het = TemporalSolver(params, timing=timing, update_delta=delta, update_kappa=kappa_oscillator)
        else:
...
def main(argv: list[str] | None = None) -> int: 
    ...
    parser.add_argument("quantity", type=str, choices=["Ca", "gs", "K"], help="quantity to drive")
    parser.add_argument("case", type=str, choices=["A", "B", "C", "D", "E"], help="tau, gamma case to explore")
    parser.add_argument("--rhomax", type=float, default=0.0, help="maximum rho value for delta/kappa profiles. If not given program will run with defaults from review.utils.constants.TemporalExploration and write to /tmp/")
    ...
    size = comm.Get_size()
    if rank == 0:
        print(f"Running with {size} rank(s) and ({args.quantity},{args.case}) for temporal exploration", flush=True)
