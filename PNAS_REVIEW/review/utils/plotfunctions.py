...
def plot_temporal_scan(quantity: str, case: str, rhomax: float = 0.5, ax: plt.Axes | None = None, vmax: float = 0.10) -> float:
    """ Plot the temporal scan results for given quantity and case """
    c = TemporalExploration()
    filename = f"files/temporal_scanning/rhomax_{rhomax:.1f}_/{quantity}/variations_{case}.txt"
    data = np.loadtxt(filename, delimiter=c.delimiter)
    periods, amplitudes = c.get_period_range(), c.get_amplitude_range() 
    if ax is None:
        fig, ax = plt.subplots(figsize=(8,6))
    heatmap = ax.pcolor(periods, amplitudes, data.T, cmap="Blues", vmin=0.0, vmax=vmax)
...
def plot_timeseries(quantity: str, case: str, amplitude: float, period: float, rhomax: float = 0.5) -> None:
    constants = TemporalExploration()
    base_path = Path(f"files/temporal_scanning/rhomax_{rhomax:.1f}_/{quantity}/{case}/")
    timeseries_hom = np.loadtxt(base_path / f"timeseries_amp{amplitude:.2f}_per{period:.2f}_hom.txt", delimiter=constants.delimiter)
    timeseries_het = np.loadtxt(base_path / f"timeseries_amp{amplitude:.2f}_per{period:.2f}_het.txt", delimiter=constants.delimiter)
    ...
    ax.plot(time, timeseries_hom, label="Homogeneous", color="blue")
    ax.plot(time, timeseries_het, label="Heterogeneous", color="orange")

    ax_ = ax.twinx()
    ax_.plot(time, variation, label="Relative Variation", color="k", linestyle=":")
    ax_.set_ylim(0, np.max([np.max(variation) * 1.1, 0.05]))
    ax_.set_ylabel("Relative Variation")
    print(f"Mean relative variation: {np.mean(variation):.4f}")
    print(f"Max relative variation: {np.max(variation):.4f}")


    ax.set_title(f"Timeseries for {quantity} - Case {case}\nAmplitude: {amplitude}, Period: {period}")
    ax.set_xlabel("Time")
    ax.set_ylabel("Assimilation rate A_n")
    ax.set_ylim(0, np.max([timeseries_hom.max(), timeseries_het.max()]) * 1.1)
    ax.legend()
    ax.grid()
    plt.show()


def plot_steady_variation(case: str, rho: tuple[float, float, float] | None = None) -> None:
    constants = TemporalExploration()
    if rho is None:
        rho = constants.rho
